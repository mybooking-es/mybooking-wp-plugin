...
<script type="text/tmpl" id="script_shiftpicker_units_option">
	<option value="<%=model.units%>">
		<%=model.units%>  <?php echo esc_html_x( 'Unit(s)', 'renting_shift_picker', 'mybooking-reservation-engine') ?>
	</option>
</script>
	...
	<li class="mybooking-rent-shift-picker-container-list-item shiftpicker-turn-item list-group-item" data-status="<%=model.availability ? 'enabled' : 'disabled'%>" data-time-from="<%=model.time_from%>" data-time-to="<%=model.time_to%>">
		...
		<div>
			<% if (model.available_units) { %>
				<small>
					<i>
						...
						&nbsp;
						<?php echo esc_html_x( 'Unit(s)', 'renting_shift_picker', 'mybooking-reservation-engine') ?> <?php echo esc_html_x( 'available', 'renting_shift_picker', 'mybooking-reservation-engine') ?>
					</i>
				</small>
			<% } %>
			<% if (!model.availability) { %>
				<small>
					<i>
						<?php echo esc_html_x( 'Unavailable', 'renting_shift_picker', 'mybooking-reservation-engine') ?>
					</i>
				</small>
			<% } %>
		</div>
...
<script type="text/tmpl" id="script_shiftpicker_info">
	<div class="card-static_resume__container mybooking-rent-shift-picker-container-info_content card">
        ...
        <?php echo esc_html_x( 'Reservation summary', 'renting_shift_picker', 'mybooking-reservation-engine') ?>
      </h2>
				...
				<strong>
					<% if (model.days > 0) { %>
						...
						<span class="time-format">
							<?php echo esc_html_x( 'day(s)', 'renting_choose_product', 'mybooking-reservation-engine' ) ?>
						</span>
					<% } else if (model.hours > 0) { %>
						...
						&nbsp;
						<span class="time-format">
							<?php echo esc_html_x( 'hour(s)', 'renting_choose_product', 'mybooking-reservation-engine' ) ?>
						</span>
					<% } else if (model.minutes > 0) { %>
						...
						&nbsp;
						<span class="time-format">
							<?php echo esc_html_x( 'minute(s)', 'renting_choose_product', 'mybooking-reservation-engine' ) ?>
						</span>
					<% } %>
				</strong>
			</div>
			<div>
				...
				&nbsp;
				<?php echo esc_html_x( 'Unit(s)', 'renting_shift_picker', 'mybooking-reservation-engine') ?>
			</div>
			<div class="date">
				<%=model.date%>
			</div>
			<div>
			<?php echo esc_html_x( 'From', 'renting_shift_picker', 'mybooking-reservation-engine') ?>
				&nbsp;
				<span class="time-from"><%=model.time_from%></span>
				&nbsp;<?php echo esc_html_x( 'to', 'renting_shift_picker', 'mybooking-reservation-engine') ?>&nbsp;
				<span class="time-to"><%=model.time_to%></span>
				&nbsp;
				<?php echo esc_html_x( 'hours', 'renting_shift_picker', 'mybooking-reservation-engine') ?>
			</div>
			<!-- // End info -->
