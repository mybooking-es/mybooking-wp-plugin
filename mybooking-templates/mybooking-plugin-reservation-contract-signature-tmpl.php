...
<script type="text/tmpl" id="script_contract_signature">
	<% if (booking.engine_sign_contract) { %>
		...
		<% if (!booking.contract_signed) { %>
			<h5>
				<?php echo esc_html_x( 'Please, sign the contract to finish the process', 'renting_my_reservation', 'mybooking-reservation-engine' ) ?>
			</h5>
			<br />
			<% if (typeof booking.electronic_signature_url !== 'undefined' && booking.electronic_signature_url && booking.electronic_signature_url != '') { %>
				<h6>
					<?php echo esc_html_x( 'Use the secure link to sign the contract', 'renting_my_reservation', 'mybooking-reservation-engine' ) ?>
				</h6>
				<hr />
				<small class="mb--txt-center">
					<?php echo esc_html_x( 'Remember to reload the page after signing the contract to download it.', 'renting_my_reservation', 'mybooking-reservation-engine' ) ?>
				</small>
		...
		<% } else { %>
			<h5>
				<?php echo esc_html_x( 'The contract has been signed successfully', 'renting_my_reservation', 'mybooking-reservation-engine' ) ?>
			</h5>
			<br />
			<% if (typeof booking.signed_contract_url !== 'undefined' && booking.signed_contract_url && booking.signed_contract_url != '') { %>
				<h6>
					<?php echo esc_html_x( 'Use the link to download the contract', 'renting_my_reservation', 'mybooking-reservation-engine' ) ?>
				</h6>
				<hr />
				<a href="<%= booking.signed_contract_url %>" target="_blank" class="mb-button block text-center">
					<?php echo esc_html_x( 'Download signed contract', 'renting_my_reservation', 'mybooking-reservation-engine') ?>
				</a>
			<% } %>
		<% } %>
	<% } else { %>
