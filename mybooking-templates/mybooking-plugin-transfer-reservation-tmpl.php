...
<script type="text/tmpl" id="script_transfer_payment_detail">
  ...
  <h4 class="brand-primary"><%= i18next.t('myReservation.pay.total_payment', {amount:configuration.formatCurrency(amount) }) %></h4>

  <% if (booking.total_paid == 0) {%>
    <div id="payment_amount_container" class="mb-alert info">
  ...
  <form name="payment_form">
    ...
    <div class="mb-form-row">
      <div class="mb-form-group mb-col-md-12">
        <button class="mb-btn mybooking-payment_button-pay" id="btn_pay" type="submit"><%= i18next.t('myReservation.pay.payment_button', {amount:configuration.formatCurrency(amount) }) %></button>
      </div>
    </div>
  </div>
