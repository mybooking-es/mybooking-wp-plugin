<?php
/**
 *   MYBOOKING ENGINE - RESERVATION SUMMARY
 *   ---------------------------------------------------------------------------
 *   The Template for showing the renting summary step
 *   This template can be overridden by copying it to your theme
 *   /mybooking-templates/mybooking-plugin-summary.php
 */
?>

<div class="mybooking-process-page">
  <section class="mybooking mybooking-process_summary mybooking-page-container <?php echo esc_attr( mybooking_engine_theme_align_width() )?>">
    <div class="mb-row">

      <!-- Reservation summary -->
      <div class="mb-col-md-12" id="reservation_detail"></div>
    </div>
  </section>
</div>

